...
#### 2.2. 探测效率 $k$ (Detection Efficiency)
...
* **决策：** 采用**最大后验概率（MAP）原则**，将初始部署点 $P_{\text{start}}$ 设定为坠毁概率分布图 $P(C)$ 中的**全局最大值点**（热点中心）。
    ...
    $$
* **理论依据 [1]：** 此策略基于时间敏感型搜索与救援的实证研究。根据 Wang 等人的研究 [1]，在采用深度强化学习进行时效性搜索任务时，**强制约束无人机从目标热点中心开始搜索**的策略，能大幅缩短搜索时间。该研究的实验量化结果表明，**若去掉“热点中心”的首步约束，找到目标的成功率降低约 38%**。因此，将初始部署点设置在 $P(C)$ 最大处，是时间紧迫的情况下最小化定位时间（TTF）的最有效、最经济的初始配置。
...
| **PSO** | `pso_search_path` | 基于粒子群优化算法，路径集中在**概率梯度最高**的区域。 | 适用于复杂概率分布，路径动态适应高价值区域。 |
